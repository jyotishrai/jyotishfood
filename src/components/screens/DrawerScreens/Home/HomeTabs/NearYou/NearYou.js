...
import { getHomeBannersAction, getNearbyRestaurantAction, getFeaturedRestaurantAction } from '../../../../../../actions/action'
...
const NearYouScreen = ({ }) => {
    ...
    const { nearbyRestaurant, fetching, error } = useSelector(state => ({
        nearbyRestaurant: state.getNearByRestaurantReducer.nearbyRestaurant,
        fetching: state.getNearByRestaurantReducer.fetching,
        error: state.getNearByRestaurantReducer.error,
    }), shallowEqual);
    ...
    function getNearByRestro(lat, lng) {
        let location = {
            lat: lat,
            lng: lng
        }
    ...
    function renderEmptyList() {
        return (
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                <RegularText
                    title={nearbyRestaurant.message}
